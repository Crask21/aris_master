    ...
    for epoch in range(first_epoch, args.num_epochs):
        ...
        if accelerator.is_main_process:
            if epoch % args.save_images_epochs == 0 or epoch == args.num_epochs - 1:
                ...
                # # -------------------------------- ADDED CODE -------------------------------- #
                # # decode the images in smaller batches to avoid OOM
                # images = images.transpose(0, 3, 1, 2) / vae.config.scaling_factor
                # decoded_images = []
                # vae_batch_size = 4  # Process 4 images at a time
                # with torch.no_grad():
                #     for i in range(0, len(images), vae_batch_size):
                #         batch = torch.tensor(images[i:i+vae_batch_size]).to(accelerator.device)
