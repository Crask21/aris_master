...
class training:
    def __init__(self):
        ...
        self.training_mode, configured_train_steps = self._resolve_training_mode_from_config()
        scheduler_training_steps = (
            len(self.dataloader) * self.config["hyperparameters"]["epochs"]
            if self.training_mode == "epochs"
            else configured_train_steps
        )
    ...
    def _resolve_training_mode_from_config(self):
        configured_steps = self.config["hyperparameters"].get("steps")
        if configured_steps is not None:
            configured_steps = int(configured_steps)
            if configured_steps <= 0:
                raise ValueError("hyperparameters.steps must be > 0 when provided")
            return "steps", configured_steps

        configured_epochs = self.config["hyperparameters"].get("epochs")
        if configured_epochs is None:
            raise ValueError("Either hyperparameters.epochs or hyperparameters.steps must be provided")
        return "epochs", None
    ...
    def load_checkpoint(self):
        ...
        if self.training_mode == "steps":
            self.max_train_steps = int(self.config["hyperparameters"]["steps"])
            self.max_train_epochs = math.ceil(self.max_train_steps / self.num_update_steps_per_epoch)
        else:
            self.max_train_epochs = int(self.config["hyperparameters"]["epochs"])
            self.max_train_steps = self.max_train_epochs * self.num_update_steps_per_epoch
        ...
        self.logger.info(f"  Num Epochs = {self.max_train_epochs}")
        ...
        if self.global_step >= self.max_train_steps:
            self.logger.info(
                f"Checkpoint already reached target steps ({self.global_step}/{self.max_train_steps})."
            )
    ...
    def train(self):
        ...
        training_start_step = self.global_step
        stop_training = self.global_step >= self.max_train_steps

        for epoch in range(self.first_epoch, self.max_train_epochs):
            if stop_training:
                break
            
            self.model.train()
            progress_total = self.num_update_steps_per_epoch
            if self.training_mode == "steps":
                progress_total = min(self.num_update_steps_per_epoch, max(self.max_train_steps - self.global_step, 0))
            self.progress_bar = tqdm(total=progress_total)
            ...
            is_last_epoch = epoch == self.max_train_epochs - 1
            if epoch % self.config["logging"]["save_images_epochs"] == 0 or is_last_epoch or stop_training:
            ...
            if epoch % self.config["logging"]["save_model_epochs"] == 0 or is_last_epoch or stop_training:
                # save the model
                unet = self.accelerator.unwrap_model(self.model)
            ...
            completed_steps = max(self.global_step - training_start_step, 1)
            avg_time_per_step = elapsed_time / completed_steps
            remaining_steps = max(self.max_train_steps - self.global_step, 0)
