...
def multi_run_evaluation(resnet18_runs_dir, output_dir=None):
    ...
    for evaluation_split, summary_filename in split_summaries.items():
        splits_data = {}
        ...
        for run_dir in run_dirs:
            ...
            # Load confusion matrix for this run
            confusion_path = run_dir / "evaluation" / f"confusion_matrix_{evaluation_split}.png"
            confusion_norm_path = run_dir / "evaluation" / f"confusion_matrix_{evaluation_split}_normalized.png"
            confusion_matrix_npy_path = run_dir / "evaluation" / f"confusion_matrix_{evaluation_split}_normalized.npy"
            cm_normalized = None
            # Try to load .npy if available, else try to reconstruct from summary
            if confusion_matrix_npy_path.exists():
                cm_normalized = np.load(confusion_matrix_npy_path)
            else:
                # Try to reconstruct from summary if present
                if "classification_report" in eval_summary:
                    # Not enough info for confusion matrix, skip
                    pass
            run_data = {
                ...
                "cm_normalized": cm_normalized.tolist() if cm_normalized is not None else None
            }

            splits_data[split_name]["runs"].append(run_data)
            logger.info(
                f"Loaded {evaluation_split} results for {split_name} run{run_number}: "
                f"acc={eval_summary['accuracy']:.4f}, f1={eval_summary['f1_score']:.4f}"
            )
        ...
        for split_name, split_data in splits_data.items():
            ...
            # Aggregate normalized confusion matrices
            cm_norms = [np.array(run["cm_normalized"]) for run in runs if run["cm_normalized"] is not None]
            if cm_norms:
                mean_cm = np.mean(cm_norms, axis=0)
                split_data["mean_normalized_confusion_matrix"] = mean_cm.tolist()
                # Save and plot
                cm_save_path = output_dir / f"mean_normalized_confusion_matrix_{split_name}_{evaluation_split}.npy"
                np.save(cm_save_path, mean_cm)
                # Plot
                ...
                plt.tight_layout()
                plot_path = output_dir / f"mean_normalized_confusion_matrix_{split_name}_{evaluation_split}.png"
                plt.savefig(plot_path, dpi=300, bbox_inches="tight")
                plt.close()
                logger.info(f"Mean normalized confusion matrix saved to: {plot_path}")
        ...
        if sorted_splits:
            create_multi_run_plot(sorted_splits, output_dir, evaluation_split=evaluation_split)
...
def create_multi_run_plot(splits_data, output_dir, evaluation_split="val"):
    """
    ...
    for real_count, splits in splits_by_real.items():
        ...
        ax1.set_title(f'Accuracy vs Synthetic Images ({real_count} Real Images, {evaluation_split.upper()})', 
        ...
        ax2.set_title(f'F1 Score vs Synthetic Images ({real_count} Real Images, {evaluation_split.upper()})', 
                     fontsize=14, fontweight='bold')
        ...
        # Save plot
        plot_path = output_dir / f"accuracy_f1_vs_synthetic_{real_count}_real_{evaluation_split}.png"
        plt.savefig(plot_path, dpi=300, bbox_inches='tight')
        add_note(
            notes_path=output_dir / "../notes.md",
            title=f"Accuracy and F1 Score vs Synthetic Images in Connected dot plot ({evaluation_split})",
            content=plot_path
        )
        plt.close()
    ...
    if len(splits_by_real) > 1:
        ...
        ax1.set_xlabel('Number of Synthetic Images', fontsize=12, fontweight='bold')
        ax1.set_ylabel('Accuracy', fontsize=12, fontweight='bold')
        ax1.set_title(f'Accuracy vs Synthetic Images (All Real Counts, {evaluation_split.upper()})', 
                     fontsize=14, fontweight='bold')
        ax1.grid(True, alpha=0.3, linestyle='--')
        ax1.set_ylim([0, 1])
        ax1.legend(loc='best')
        ...
        ax2.set_xlabel('Number of Synthetic Images', fontsize=12, fontweight='bold')
        ax2.set_ylabel('F1 Score', fontsize=12, fontweight='bold')
        ax2.set_title(f'F1 Score vs Synthetic Images (All Real Counts, {evaluation_split.upper()})', 
                     fontsize=14, fontweight='bold')
        ax2.grid(True, alpha=0.3, linestyle='--')
        ax2.set_ylim([0, 1])
        ax2.legend(loc='best')
        ...
        combined_plot_path = output_dir / f"accuracy_f1_vs_synthetic_combined_{evaluation_split}.png"
        plt.savefig(combined_plot_path, dpi=300, bbox_inches='tight')
        
        add_note(
            notes_path=output_dir / "../notes.md",
            title=f"Combined Accuracy and F1 Score vs Synthetic Images in Connected dot plot ({evaluation_split})",
...
def bar_chart(splits_data, output_dir, evaluation_split="val"):
    """
    ...
    plt.title(f'Mean Accuracy Across Splits ({real_count} Real Images, {evaluation_split.upper()})', fontsize=14, fontweight='bold')
    ...
    bar_chart_path = output_dir / f"mean_accuracy_bar_chart_{evaluation_split}.png"
    plt.savefig(bar_chart_path, dpi=300)
    add_note(
            notes_path=output_dir / "../notes.md",
            title=f"Mean Accuracy Bar Chart ({evaluation_split})",
